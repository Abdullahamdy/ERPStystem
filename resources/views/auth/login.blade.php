...
.form-control-feedback {
   ...
   /* General styles for toggle button */
    .toggle-password-btn {
        position: absolute;
        top: 50%;
    ...
    /* LTR Style - Button on the left */
    html[dir="rtl"] .toggle-password-btn {
        left: 0;
        
    }

    /* RTL Style - Button on the right */
    html[dir="ltr"] .toggle-password-btn {
        right: 0;
    }

    /* Adjust input padding for RTL and LTR */
    html[dir="ltr"] #password {
        padding-left: 40px; /* Space for the button on the left */
        padding-right: 15px; /* Default padding on the right */
    }

    html[dir="rtl"] #password {
        padding-right: 40px; /* Space for the button on the right */
        padding-left: 15px; /* Default padding on the left */
    }
</style>

@section('content')
    <div class="login-form col-md-12 col-xs-12 right-col-content" style="margin-top:10px;">
...
<div class="form-group has-feedback {{ $errors->has('password') ? ' has-error' : '' }}" style="position: relative;">
    <button type="button" id="toggle-password-visibility" class="btn btn-secondary toggle-password-btn">
        👁️
    </button>
    <input id="password" type="password" class="form-control" name="password"
           value="{{ $password }}" required placeholder="@lang('lang_v1.password')" style="padding-left: 40px; padding-right: 40px;">
...
<script>
    document.getElementById('toggle-password-visibility').addEventListener('click', function () {
        ...
        var toggleButton = document.getElementById('toggle-password-visibility');

        if (passwordInput.type === 'password') {
            passwordInput.type = 'text';
            toggleButton.textContent = '🙈'; // Change icon or text to indicate hidden state
        } else {
            passwordInput.type = 'password';
            toggleButton.textContent = '👁️'; // Change icon or text to indicate visible state
        }
    });
</script>
