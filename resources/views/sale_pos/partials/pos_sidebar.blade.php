    ...
    @if (!empty($categories))
        @foreach ($categories as $category)
            <div class="col-md-3 col-xs-4 ">
                <div class="product_category_box" data-variation_id="{{ $category['id'] }}" title="Dammam  (23ss) "
                    style="
    width: 100%;
    margin-bottom: 10px;
    text-align: center;
...
">
                    ...
                    <div class="image-container"
                        style="    background-image: url(http://localhost:8000/uploads/img/1727787721_ww.jpeg);
            ...
            <tr class="cursor-pointer"
                ...
                <td>
                    ...
                    @if (auth()->user()->can('sell.delete') || auth()->user()->can('direct_sell.delete'))
                        <a href="{{ action('SellPosController@destroy', [$transaction->id]) }}" class="delete-sale"
                            style="padding-left: 20px; padding-right: 20px"><i class="fa fa-trash text-danger"
                                title="{{ __('lang_v1.click_to_delete') }}"></i></a>
                    @endif

                    <a href="{{ action('SellPosController@printInvoice', [$transaction->id]) }}"
                        class="print-invoice-link">
